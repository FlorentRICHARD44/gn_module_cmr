DROP VIEW IF EXISTS gn_cmr.v_cmr_sitegroup_observations_cmr_test;
CREATE OR REPLACE VIEW gn_cmr.v_cmr_sitegroup_observations_cmr_test
 AS
 ...
 SELECT v.date::date AS "visit_date",
  ...
  (sg.data#>>'{zoneprotegee}')::text as "zone_protegee",
    s.name AS "point_de_capture",
	s.id_site as "id_site",
  (s.data#>>'{landuse}')::text as "type_terrain",
	s.geom as "geom",
    ...
    vo.v_observers AS "operateurs",
        CASE
            ...
            ELSE 'Non'::text
        END AS "observation",
    (v.data#>>'{conditions_climat}')::text as "conditions_climat",
    i.id_individual as "id_individual",
    i.identifier AS "numero_individu",
    i.rfid AS "rfid",
    i.marker AS "marquage",
    (i.data#>>'{test}')::text as "test_champ",
	o.id_observation as "id_observation",
  (o.data#>>'{test}')::text as "test_type",
  (o.data#>>'{test_gp_a}')::text as "test_text",
  (o.data#>>'{test_gp_b}')::text as "test_select"
...
-- Exemple de vue avec récupération de valeur dans les colonnes JSONB
-- Attention aux formats de date, et au valeurs non requises qui peuvent donner des NULL.
